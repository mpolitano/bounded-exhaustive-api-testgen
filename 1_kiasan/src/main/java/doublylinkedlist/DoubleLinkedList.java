...
public class DoubleLinkedList implements java.io.Serializable {
    //@ invariant repOK();
    ...
    public static IFinitization finDoubleLinkedList(int maxSize) {
        return finDoubleLinkedList(maxSize, 0, maxSize, 0, maxSize);
    }
    
    public static IFinitization finDoubleLinkedList(int numEntries,int minSize, int maxSize, int minKey,int maxKey) {
        ...
        IIntSet size = f.createIntSet(minSize, maxSize);

        f.set("DoubleLinkedList.header", entries);
        f.set("DoubleLinkedList.size", size);
        f.set("Entry.next", entries);
        f.set("Entry.previous", entries);
        f.set("Entry.element", values);
    ...
    public boolean repOK() {
        ...
        do {
            ...
            //Avoid null. Add for Mariano. It is no a bug, only for equals to BE
            if (tmp != header && tmp.element == null) {
                return false;
            }
    ...
    public class Entry {
       ...
       public boolean nonNullPointers() {
