...
// 		return true;
	...
	public static IFinitization finNodeCachingLinkedList(int size){
		return finNodeCachingLinkedList(size, 0, size, size);
	}

	public static IFinitization finNodeCachingLinkedList(int numEntries, int minSize, int maxSize,
			int numElems) {
		...
		IIntSet msizes = f.createIntSet(0,maxSize);
		
		f.set("header", entries);
		f.set("size", sizes);

		// f.set("maximumCacheSize",msizes);
		f.set("firstCachedNode", entries);
		f.set("cacheSize", msizes);
