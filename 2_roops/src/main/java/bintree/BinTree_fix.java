...
public class BinTree_fix implements java.io.Serializable{
	...
	public static IFinitization finBinTree_fix	(int numNodes) throws Exception {

		IFinitization f = FinitizationFactory.create(BinTree_fix.class);

		//  IObjSet nodes = f.createObjSet(Node.class, numNodes);
		//  nodes.setNullAllowed(true);


		IObjSet nodes = f.createObjSet(BinTreeNode.class,numNodes);
		nodes.setNullAllowed(true);
//		IClassDomain entryClassDomain = f.createClassDomain(BinTreeNode.class);
//		nodes.addClassDomain(entryClassDomain);
//		entryClassDomain.includeInIsomorphismCheck(false);

		// IIntSet sizes = f.createIntSet(minSize, maxSize);
		IIntSet values = f.createIntSet(0, numNodes-1);

		f.set("root", nodes);
	    f.set("BinTreeNode.parent", nodes);

		f.set("BinTreeNode.left", nodes);
		f.set("BinTreeNode.right", nodes);
		f.set("BinTreeNode.key", values);
